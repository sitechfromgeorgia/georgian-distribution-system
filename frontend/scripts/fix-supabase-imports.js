...
function fixFileImports(filePath) {
  ...
  // Pattern 2: Fix old supabase.ts re-export imports
  if (content.includes("from '@/lib/supabase'") && content.includes('{ supabase }')) {
    // This needs manual checking - might be our new barrel export
    // We'll skip auto-fixing this
  }

  // Pattern 3: Fix client-fixed imports
  ...
  // Pattern 4: Fix relative imports to supabase/client or supabase/server
  content = content.replace(
  ...
  // Pattern 5: Fix old './supabase' relative imports
  content = content.replace(
  ...
  // Pattern 6: Fix direct supabase/client or supabase/server imports to use barrel export
  content = content.replace(
  ...
  // Pattern 7: Fix singleton supabase instances
