...
class QueryOptimizer {
   ...
   */
  async batchQueries(queries: Array<{ table: string; select: string; filter?: any }>): Promise<any[]> {
    const startTime = Date.now();

    try {
      const promises = queries.map(async (q) => {
        let query = this.supabase.from(q.table).select(q.select);
   ...
   */
  async searchProductsOptimized(searchTerm?: string, category?: string, limit = 50): Promise<any[]> {
    const startTime = Date.now();
  ...
  private async getDashboardAnalyticsFallback(): Promise<any> {
    ...
    const { data: restaurants } = await this.supabase.from('profiles').select('*').eq('role', 'restaurant');
    ...
    return {
      ...
      totalRevenue: orders?.reduce((sum, order) => sum + ((order as any).total_amount || 0), 0) || 0
    };
  }
